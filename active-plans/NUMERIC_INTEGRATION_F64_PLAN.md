...
`math_expressions::precise::integrate_to_precision(f, x, a, b, digits)`
(`src/precise/quad.rs`) is a **certified** definite-integral routine: it returns
a value only with a *proven* worst-case error bound at or below the requested
...
digit even *at* 13 — e.g. `∫₀^π sin` certifies `2.00000000000` at 12 digits but
**refuses at 13** (the true value is 2 to ~30 digits; the *certified* bound is
...
f64 nodes ⇒ sub-f64 certification. Saturating f64 requires computing *above*
f64 internally.
...
`MpFix`, Ziv-escalated). That is *exact to any requested digits* — it bypasses
...
- **Requires:** an extended-precision *interval* type for both node values and
  ...
  extended-precision *nodes* are the key; the remainder can also be driven small
...
- **Gauss–Kronrod** (G7–K15) — cheap embedded error estimate, but *rigorous*
...
2. **Phase 2 (Lever 2 + compensated summation)** for robust f64 saturation on
   *true* non-elementary integrands; raise the 13 cap afterward.
...
- `src/precise/quad.rs` — Lever 1 fast path in `integrate_to_precision`;
  Lever 2 extended-precision node evals + interval type; Lever 3 rule.
- `src/precise/fix.rs` / a new `iv_dd.rs` — extended-precision interval type
  (Lever 2).
- `src/precise/mod.rs` — reuse `eval_tape` at a fixed working precision for
  nodes (Lever 2).
