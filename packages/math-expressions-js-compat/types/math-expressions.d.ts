...
// contract, checked by `npm run typecheck` for internal consistency and by
// DoenetML's ~147 call sites for usefulness, but not derived from the
// implementation. Where the Rust engine genuinely diverges from it, the
// divergence belongs in the release notes, not in a widened type that hides it.
//
