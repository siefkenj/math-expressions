// Pure-JS polynomial / Groebner-basis engine, ported faithfully from the legacy
// `lib/polynomial/polynomial.js`. There is no Rust/wasm backing for this API, so
// the original algorithm is reproduced verbatim (TypeScript-annotated with
// `any`). Coefficient arithmetic and `simplify` are routed through the compat
// modules; `evaluate_to_constant` is the lean wasm-backed shim.
...
import * as sv from "./single-var-poly";
...
// Local replacement for underscore's `_.isEqual` (deep structural equality),
// used only for fixpoint detection in `reduce` / `pt_reduce`.
...
function expression_to_polynomial(expr_or_tree: any): any {
  ...
  if (operator === "+") {
    ...
    if (!result.every((v: any) => v !== false)) return false;

    return result.reduce((u: any, v: any) => polynomial_add(u, v));
  } else if (operator === "-") {
    ...
    return result.reduce((u: any, v: any) => polynomial_mul(u, v));
  } else if (operator === "^") {
    let base = operands[0];
    ...
    if (typeof pow !== "number" || pow < 0 || !Number.isInteger(pow)) {
      ...
      if (Number.isFinite(pow_num)) {
        ...
        if (pow_fraction.d <= 100) {
          ...
          var results: any = ["polynomial", simplify.simplify(base), []];
...
function poly_to_terms(f: any): any {
  if (!Array.isArray(f) || f[0] !== "polynomial") {
    return f;
  }

  let terms: any = ["polynomial_terms"];
  let focus: any = f;
  let var_powers: any[] = [];
  let current_term: any = [];
  ...
  while (f !== 0) {
    ...
    while (Array.isArray(focus) && focus[0] === "polynomial") {
      let x = focus[1];
      let terms2 = focus[2];
      let exp = terms2[terms2.length - 1][0];
      focus = terms2[terms2.length - 1][1];
...
function terms_to_poly(f: any): any {
...
function polynomials_in_same_leading_variable(p: any, q: any): any {
...
function polynomial_add(p: any, q: any): any {
...
function terms_poly_add(p: any, q: any): any {
  let sum: any = [];
  let term: any = [];
...
function polynomial_neg(p: any): any {
  if (p[0] !== "polynomial") {
    return simplify.simplify(["-", p]);
  }

  let result: any = ["polynomial", p[1], []];
  let p_terms = p[2];
...
function terms_poly_neg(p: any): any {
  if (!Array.isArray(p) || p[0] !== "polynomial_terms") {
    return simplify.simplify(["-", p]);
  }

  let result: any = ["polynomial_terms"];
  let len = p.length;
...
function polynomial_sub(p: any, q: any): any {
  return polynomial_add(p, polynomial_neg(q));
}

function terms_poly_sub(p: any, q: any): any {
  return terms_poly_add(p, terms_poly_neg(q));
}

function polynomial_mul(p: any, q: any): any {
  if (p[0] !== "polynomial") {
    if (q[0] !== "polynomial") {
      return simplify.simplify(["*", p, q]);
    } else if (p) {
      let prod: any = ["polynomial", q[1], []];
      let q_terms = q[2];
...
function mono_mul(p: any, q: any): any {
...
function terms_poly_mul(p: any, q: any): any {
  if (!Array.isArray(p) || p[0] !== "polynomial_terms") {
    if (!Array.isArray(q) || q[0] !== "polynomial_terms")
      return simplify.simplify(["*", p, q]);
    let prod: any = ["polynomial_terms"];
    let len_q = q.length;
...
function polynomial_pow(p: any, e: any): any {
...
function polynomial_to_expression(p: any): any {
...
function initial_term(p: any): any {
...
function pt_initial_term(p: any): any {
  if (!Array.isArray(p) || p[0] !== "polynomial_terms") return p;

  return p[1];
}

function mono_less_than(left: any, right: any): boolean {
...
function mono_gcd(left: any, right: any): any {
...
function mono_div(top: any, bottom: any): any {
...
function mono_is_div(top: any, bottom: any): boolean {
...
function mono_to_poly(mono: any): any {
...
function max_div_init(f: any, monos: any): any {
  ...
  let current_term: any = ["monomial", focus, var_powers];
...
function pt_max_div_init(f: any, monos: any): any {
  if (f === 0) return 0;
...
function poly_div(f: any, divs: any): any {
  ...
  let m2 = max_div_init(f_prime, inits);

  while (m2 !== 0) {
    sp = m2[1];
    mp = mono_div(m2[0], inits[sp]);
    su_mu.push([sp, mp]);
    f_prime = polynomial_sub(
      f_prime,
      polynomial_mul(mono_to_poly(mp), divs[sp]),
    );
    m2 = max_div_init(f_prime, inits);
  }

  return [su_mu, f_prime];
}

function pt_poly_div(f: any, divs: any): any {
  let inits: any[] = [];
  let su_mu: any[] = [];
  let sp: any = [];
  let mp: any = [];
  ...
  let m2 = pt_max_div_init(f_prime, inits);
  ...
  while (m2 !== 0) {
    ...
    m2 = pt_max_div_init(f_prime, inits);
  }

  return [su_mu, f_prime];
}

function prereduce(polys: any): any {
...
function pt_prereduce(polys: any): any {
  //takes an array of polys, and does some simply reductions: gets rid of 0 polynomials, if there's a constant: just return [1], if there are no nonzero polys: return [0].
...
function reduce_ith(i: any, polys: any): any {
...
function pt_reduce_ith(i: any, polys: any): any {
  //takes an index i and an array polys of polynomials, and reduces the ith polynomial wrt the rest (i.e., finds a std expression and replaces with f'). Returns the reduced polynomial.
  //stringify vars before calling this
...
function pt_reduce(polys: any): any {
  //takes an array of polynomials, and reduces them with respect to each other until they can't be reduced anymore. Returns array of reduced polynomials.
  //stringify vars before calling this
...
function hij(i: any, j: any, polys: any): any {
...
function pt_hij(i: any, j: any, polys: any): any {
  //takes indices i,j and an array of polynomials. Returns the polynomial hij computed from the ith and jth polys in polynomials, for Buchberger's criterion.
  //stringify vars before calling this
...
function build_hij_table(table: any, polys: any): any {
  ...
  while (j < len) {
    while (i < j) {
      table[[i, j] as any] = hij(i, j, polys);
      if (table[[i, j] as any] !== 0) candidates.push(table[[i, j] as any]);
      i = i + 1;
    }
    i = 0;
    j = j + 1;
...
function pt_build_hij_table(table: any, polys: any): any {
  //takes an empty table and an array of polynomials, and fills in table where table[[i,j]]=h_ij for i < j < polys.length. (Look up h_ij based on the index in polys) returns first choice for h
  //stringify vars before calling this
  ...
  while (j < len) {
    while (i < j) {
      table[[i, j] as any] = pt_hij(i, j, polys);
      if (table[[i, j] as any] !== 0) candidates.push(table[[i, j] as any]);
      i = i + 1;
    }
    i = 0;
...
function update_hij_table(table: any, polys: any, h: any): any {
  ...
  while (i < len) {
    ...
    table[[i, len] as any] = hij(i, len, polys);
    if (table[[i, len] as any] !== 0) candidates.push(table[[i, len] as any]);
    i = i + 1;
  }
...
function pt_update_hij_table(table: any, polys: any, h: any): any {
  //takes an hij table, array of polynomials that the table was built for, and a poynomial h. Updates the hij values for the addition of the polynomial h, h is added with the highest index. return next choice of h.
  ...
  while (i < len) {
    //this loop adds values with h
    table[[i, len] as any] = pt_hij(i, len, polys);
    if (table[[i, len] as any] !== 0) candidates.push(table[[i, len] as any]);
    i = i + 1;
  }
...
function choice_first(polys: any): any {
  return polys[0];
}

function choice_lowest_degree(polys: any): any {
...
function pt_choice_lowest_degree(polys: any): any {
  //takes an array of polynomials, returns the polynomial of lowest degree.
...
function reduced_grobner(polys: any): any {
...
function pt_reduced_grobner(polys: any): any {
  //takes an array of polynomials, returns reduced grobner basis of the ideal they generate.
  //stringify vars before calling this
...
function pt_poly_lcm(f: any, g: any): any {
  //takes two polynomials f and g, and returns their least common multiple.
  //stringify vars before calling this

  let t: any = ["polynomial_terms", ["monomial", 1, [["_t", 1]]]];
  let one_minus_t: any = ["polynomial_terms", ["monomial", -1, [["_t", 1]]], 1];
...
function poly_gcd(f: any, g: any): any {
...
function pt_poly_gcd(f: any, g: any): any {
  //takes two polynomials f and g, and returns their greatest common divisor.
  //stringify vars before calling this
...
function poly_by_divisor(f: any, d: any): any {
...
function pt_poly_by_divisor(f: any, d: any): any {
  //takes two polynomials f and d (where d is a divisor of f), and returns f divided by d.
  //!!Only call if d evenly divides f!!
  //stringify vars before calling this
...
function reduce_rational_expression(top: any, bottom: any): any {
...
function pt_reduce_rational_expression(top: any, bottom: any): any {
  //input: top and bottom of a rational expression. top and bottom should be polynomials, ["polynomial", ...]. returns an array with two entries: new_top and new_bottom, which are reduced (gcd of new_top and new_bottom is 1). new_bottom will always have leading coefficient 1 (according to lexicographic order)
...
function stringify_vars(f: any): any {
...
function destringify_vars(f: any, table: any): any {
