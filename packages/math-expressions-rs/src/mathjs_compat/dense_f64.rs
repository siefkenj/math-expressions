//! Dense f64 linear algebra: the mathjs `lusolve` and `eigs` replacements
//! Doenet consumes (Gaussian elimination; Hessenberg + shifted-QR
//! eigendecomposition with null-vector eigenvectors).
//!
//! All loops are bounded (fixed iteration caps, no wall-clock); failures are
//! `None`, never panics — this module is wasm-boundary-facing.
